...
from utils import set_seed, gen_local_tokens, gen_router, route_and_pack_padding_free, alloc_shmem_buffers, nvtx_push, nvtx_pop, _allreduce_max_i32, _allreduce_min_i32, _sync_and_check, _build_dst_offsets, _masked_stats, reset_custom_iter_state
...
def check_compare(
    ...
    # genegerate expert stuff
    expert_hidden_dim = hidden_dim * 4  # normally MLP 4 times
    weights = torch.randn(
    ...
    for a2a_out in range(ITERS):
        ...
        # clean Padding，do the shuffle
        packed_tokens = shuffle(a2a_out, buffers.pca)

        # do Grouped GEMM
        # calc the excate Token sum of the local experts
        local_expert_cnt = buffers.pca.sum(dim=1).to(torch.int32)
        # calc the shuffled tokens and weights
        expert_out = expert(packed_tokens, weights, local_expert_cnt, num_experts_total)
        dist.barrier()
