...
def _alloc_symm_gather_buffers(
    ...
    # symmem.rendezvous requires every rank to pass exactly the same shape.
    # Each rank's total_rows (the number of tokens sent by that rank) differs
    # because MoE routing is uneven, so align the allocation to the global max.
    # This preserves padding-free semantics: symm_tokens is the send-side buffer
    # for pull-based shmem, and remote kernels use count metadata to read only
    # the valid symm_tokens[:total_rows] rows. The aligned tail is never read
    # and consumes no communication bandwidth because rendezvous exchanges
    # pointers rather than data.
    total_rows_t = torch.tensor([total_rows], dtype=torch.int64, device=device)
    dist.all_reduce(total_rows_t, op=dist.ReduceOp.MAX)
    total_rows_aligned = int(total_rows_t.item())

    symm_tokens = symmem.empty((total_rows_aligned, hidden_dim), dtype=token_dtype, device=device)
    ...
    # Copy only this rank's valid rows; the aligned tail is never read.
    symm_tokens[:total_rows].copy_(tokens)
...
def wait_counts_ready(
    ...
    *,
    ...
    """
    rank = dist.get_rank()
    print(f"[rank{rank}] ENTER wait_counts_ready", flush=True)
    ...
    while True:
        ...
        if cur >= target:
            print(f"[rank{rank}] EXIT wait_counts_ready", flush=True)
            return
...
def wait_token_sync_ready(
    ...
    *,
    ...
    """
    rank = dist.get_rank()
    print(f"[rank{rank}] ENTER wait_token_sync_ready", flush=True)
    ...
    while True:
        ...
        if ready:
            print(f"[rank{rank}] EXIT wait_token_sync_ready", flush=True)
            return
