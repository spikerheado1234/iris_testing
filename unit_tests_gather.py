...
from layers.expert import expert_overlap
...
def test_gather_a2a(e_local: int = 2, hidden_dim: int = 128, cap: int = 32, threshold: float = 1e-2):
    ...
    print(f"[rank{rank}] before alloc_symm", flush=True)
    ...
    print(f"[rank{rank}] after alloc_symm", flush=True)
    dist.barrier()
    print(f"[rank{rank}] after barrier_before_gather", flush=True)
    gathered = gather_a2a(
        symm["cnts"],
        symm["offsets"],
        symm["tokens"],
    ...
    print(f"[rank{rank}] after gather_a2a", flush=True)
    torch.cuda.synchronize()
    print(f"[rank{rank}] after cuda_sync", flush=True)
    dist.barrier()
    print(f"[rank{rank}] after final_barrier", flush=True)
...
def ref_overlap_expert(tokens, weights, counts_by_src, write_meta):
    """
    tokens: [S, H], layout follows write_meta[src, e]
    weights: [E, H, N]
    counts_by_src: [W, E]
    write_meta: [W, E]
    """
    W, E = counts_by_src.shape
    N = weights.shape[-1]
    out = torch.empty((tokens.shape[0], N), device=tokens.device, dtype=torch.float32)

    for e in range(E):
        for src in range(W):
            c = int(counts_by_src[src, e].item())
            if c <= 0:
                continue
            off = int(write_meta[src, e].item())
            out[off:off + c] = tokens[off:off + c].float() @ weights[e].float()

    return out


def build_write_meta_expert_major(counts_by_src):
    """
    counts_by_src: [W, E]
    output layout: expert-major, then src-major.
    """
    W, E = counts_by_src.shape
    write_meta = torch.empty_like(counts_by_src, dtype=torch.int64)

    running = 0
    for e in range(E):
        for src in range(W):
            write_meta[src, e] = running
            running += int(counts_by_src[src, e].item())

    return write_meta.contiguous()


def test_expert_overlap_single_gpu(
    W=4,
    E=4,
    H=256,
    N=1024,
    ...
    assert counts_by_src.shape == (W, E)

    write_meta = build_write_meta_expert_major(counts_by_src)
    total_tokens = int(counts_by_src.sum().item())

    tokens = torch.randn((total_tokens, H), device=device, dtype=dtype)
    weights = torch.randn((E, H, N), device=device, dtype=dtype)

    max_cnt = int(counts_by_src.max().item())
    max_tiles = triton.cdiv(max(max_cnt, 1), block_m)

    ready = torch.ones((E, W, max_tiles), device=device, dtype=torch.int32)

    state = SimpleNamespace(
        gathered_tokens=tokens,
        local_expert_cnts=counts_by_src,
    ...
    out = expert_overlap(state, weights, W * E)
    torch.cuda.synchronize()

    ref = ref_overlap_expert(tokens, weights, counts_by_src, write_meta)
    out_f = out.float()

    max_diff = (out_f - ref).abs().max().item() if out.numel() > 0 else 0.0
    print(
        f"[EXPERT_OVERLAP] W={W} E={E} H={H} N={N} "
        f"total_tokens={total_tokens} max_diff={max_diff:.6f}",
        flush=True,
    )

    assert out_f.shape == ref.shape
    assert torch.allclose(out_f, ref, atol=atol, rtol=rtol), (
        f"expert_overlap mismatch, max_diff={max_diff}"
    )
