...
from  layers.all_to_all_scatter import custom_a2a, _LAST_DBG
    ...
    try:
        for i in range(PROFILE_ITERS):
            ...
            _ = custom_a2a(
                ...
                buffers.debug_time, # NEW TIMING
                buffers.recv_wait_cycles, # TO OP
                buffers.heap_bases,
                num_experts_total,
                capacity,
...
def check_compare(
    ...
    # NEW RECORED THE AVG AND MAX OF ALL iterations
    accum_debug_time = torch.zeros((world_size, e_local, 3), dtype=torch.int64, device=device)
    max_debug_time = torch.zeros((world_size, e_local, 3), dtype=torch.int64, device=device)
    accum_recv_wait = torch.zeros((e_local,), dtype=torch.int64, device=device)
    max_recv_wait = torch.zeros((e_local,), dtype=torch.int64, device=device)
    ...
    for i in range(ITERS):
        ...
        buffers.token_sync.zero_()
        buffers.tile_counter.zero_()
        buffers.debug_time.zero_() #NEW Clear the debug time
        buffers.recv_wait_cycles.zero_()
        ...
        # NEW accumulate all data from iters
        cur_debug = buffers.debug_time.detach()
        accum_debug_time += cur_debug
        max_debug_time = torch.maximum(max_debug_time, cur_debug)

        # ALSO ACCUMULATE Receiver Wait
        cur_recv = buffers.recv_wait_cycles.detach()
        accum_recv_wait += cur_recv
        max_recv_wait = torch.maximum(max_recv_wait, cur_recv)
   ...
   # Align start (do once not in the per-iter)
    ...
    # NEW full Spin-lock Breakdown Report (Strict Hardware Cycles)
    
   # 1. Gather global stats (Sender)
    all_accum = [torch.zeros_like(accum_debug_time) for _ in range(world_size)]
    all_max   = [torch.zeros_like(max_debug_time) for _ in range(world_size)]
    dist.all_gather(all_accum, accum_debug_time)
    dist.all_gather(all_max, max_debug_time)

    # 2. Gather global stats (Receiver)
    all_recv_accum = [torch.zeros_like(accum_recv_wait) for _ in range(world_size)]
    all_recv_max   = [torch.zeros_like(max_recv_wait) for _ in range(world_size)]
    dist.all_gather(all_recv_accum, accum_recv_wait)
    dist.all_gather(all_recv_max, max_recv_wait)

    if rank == 0:
        global_avg_cycles = torch.stack(all_accum).float() / ITERS
        global_max_cycles = torch.stack(all_max).float()
        
        global_recv_avg = torch.stack(all_recv_accum).float() / ITERS
        global_recv_max = torch.stack(all_recv_max).float()

        block_totals_avg = global_avg_cycles.sum(dim=-1)
        valid_mask = block_totals_avg > 0

        if valid_mask.any():
            spin_avg = global_avg_cycles[:, :, :, 1][valid_mask]
            spin_max = global_max_cycles[:, :, :, 1][valid_mask]
            valid_block_totals = block_totals_avg[valid_mask]

            # # Rigorous block-work waste ratio (Sender-side only)
            total_spin_cycles = spin_avg.sum().item()
            total_block_cycles = valid_block_totals.sum().item()
            sender_spin_ratio = (total_spin_cycles / total_block_cycles) * 100 if total_block_cycles > 0 else 0
            
            # Hardware Time Approximation (Nominal 100MHz RTC = 1e-5 ms/cycle)
            CYCLES_TO_MS = 1e-5 
            
            sender_mean_cycles = spin_avg.mean().item()
            recv_mean_cycles = global_recv_avg.mean().item()
            
            sender_max_ms = spin_max.max().item() * CYCLES_TO_MS
            recv_max_ms = global_recv_max.max().item() * CYCLES_TO_MS
            
            total_custom_ms = custom_ms_max.item()

            print("\n" + "="*75)
            print("=== Instrumented Spin-Lock Breakdown (Cycle-Based Approximation) ===")
            print("="*75)
            
            print("[1] End-to-End Wall-clock Runtime:")
            print(f"    Custom A2A (Push + Receiver Sync) : {total_custom_ms:.3f} ms")
            
            print("\n[2] Sender-Side Wait (Local Completion Spin):")
            print(f"    Global Sender Spin Work Ratio     : {sender_spin_ratio:.2f}% (Total spin / Total active block cycles)")
            print(f"    Mean Spin Delay per active block  : ~{sender_mean_cycles * CYCLES_TO_MS:.3f} ms ({sender_mean_cycles:.0f} cycles)")
            print(f"    Absolute MAX Delay on worst block : ~{sender_max_ms:.3f} ms ({spin_max.max().item():.0f} cycles)")
            
            print("\n[3] Receiver-Side Wait (Remote Sync Polling):")
            print(f"    Mean Sync Polling per expert      : ~{recv_mean_cycles * CYCLES_TO_MS:.3f} ms ({recv_mean_cycles:.0f} cycles)")
            print(f"    Absolute MAX Delay on worst expert: ~{recv_max_ms:.3f} ms ({global_recv_max.max().item():.0f} cycles)")
            print("    *Note: ms values are rough approximations using nominal AMD RTC clock @ 100MHz.")

            print("\n[4] Top 5 Most Expensive Sender Spin-Locks (By Avg Cycles):")
            flat_spin_avg = global_avg_cycles[:, :, :, 1].flatten()
            flat_block_total = block_totals_avg.flatten()
            
            valid_flat_idx = torch.nonzero(flat_block_total > 0, as_tuple=False).flatten()
            valid_spin_vals = flat_spin_avg[valid_flat_idx]
            
            k = min(5, valid_spin_vals.numel())
            if k > 0:
                topk_vals, topk_pos = torch.topk(valid_spin_vals, k)
                topk_indices = valid_flat_idx[topk_pos]
                
                for i in range(k):
                    val = topk_vals[i].item()
                    idx = topk_indices[i].item()
                    
                    rec_r  = idx // (world_size * e_local)
                    rem    = idx % (world_size * e_local)
                    dst_r  = rem // e_local
                    exp_id = rem % e_local
                    
                    pct_of_block = (val / flat_block_total[idx].item()) * 100 if flat_block_total[idx].item() > 0 else 0
                    
                    print(f" #{i+1} | Recording Rank {rec_r} -> Dst Rank {dst_r} [Expert {exp_id}]")
                    print(f"     | Avg Spin: {val:.0f} cycles ({pct_of_block:.2f}% of this block's lifecycle)")
            print("="*75 + "\n")
